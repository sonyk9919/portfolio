...
import { trackLinkClick, trackProjectOpen } from "@/lib/analytics";
...
export const ProjectAccordion = ({ project, open, onToggle }: Props) => {
  const handleToggle = () => {
    if (!open) trackProjectOpen(project.slug, project.title);
  ...
  return (
    ...
    >
      ...
      >
        {project.problem && (
          <p
            className="border-l-2 pl-4 text-[0.95rem] leading-relaxed text-neutral-700 dark:text-neutral-300"
          ...
          <div className="mt-5 flex flex-wrap gap-4">
            {project.links.map((link) => (
              <a
                ...
                onClick={() => trackLinkClick(link.label, link.href)}
